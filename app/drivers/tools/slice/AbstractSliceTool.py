...
class AbstractSliceTool(AbstractTool):
    ...
    def emit_normal(self, message):
        super().emit_normal("slice-tool", self.name, message)

    def emit_warning(self, message):
        super().emit_warning("slice-tool", self.name, message)

    def emit_error(self, message):
        super().emit_error("slice-tool", self.name, message)

    def emit_highlight(self, message):
        super().emit_highlight("slice-tool", self.name, message)

    def emit_success(self, message):
        super().emit_success("slice-tool", self.name, message)

    def emit_debug(self, message):
        super().emit_debug("slice-tool", self.name, message)
