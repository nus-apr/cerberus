...
class FuzzRepairValidate(AbstractRepairTool):
    def __init__(self):
        self.name = os.path.basename(__file__)[:-3].lower()
        super().__init__(self.name)
        self.image_name = "yuntongzhang/fuzzrepair:validation"
        self.bug_id = ""
    ...
    def instrument(self, bug_info: Dict[str, Any]) -> None:
        ...
        self.dir_inst = join(self.dir_setup, "fuzzrepair")
        super(FuzzRepairValidate, self).instrument(bug_info)
        return

    def run_repair(self, bug_info, repair_config_info):
        config_script = bug_info.get(self.key_config_script, None)
        build_script = bug_info.get(self.key_build_script, None)
        bug_id = bug_info.get(self.key_bug_id, None)
        subject = bug_info.get(self.key_subject, None)
        ...
        super(FuzzRepairValidate, self).run_repair(bug_info, repair_config_info)
    ...
    def save_artifacts(self, dir_info):
        # the outputs have already been saved to /output, by the tool
        super(FuzzRepairValidate, self).save_artifacts(dir_info)
        return
