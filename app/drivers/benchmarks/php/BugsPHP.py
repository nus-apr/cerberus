...
class BugsPHP(AbstractBenchmark):

    log_instrument_path = None

    def __init__(self) -> None:
        self.name = os.path.basename(__file__)[:-3].lower()
        super(BugsPHP, self).__init__()
    ...
    def setup_container(
        ...
        """
        container_id = super(BugsPHP, self).setup_container(
    ...
    def build(self, bug_index: int, container_id: Optional[str]) -> bool:
        ...
        subject_id = str(experiment_item[self.key_bug_id]).split("-")[-1]
        bug_id = str(experiment_item[self.key_bug_id])
        command_str = f"bash build_subject {subject_id}"
        self.log_build_path = (
    ...
    def save_artifacts(
        ...
        super(BugsPHP, self).save_artifacts(dir_info, container_id)
