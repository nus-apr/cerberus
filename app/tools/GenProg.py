...
class GenProg(AbstractTool):
        ...
        if not instrument_only:
            ...
            repair_config_str = (
                ...
                "--test-script bash {dir_exp}/test.sh\n".format(
                    bug_id=bug_id, p_size=count_pass, n_size=count_neg, dir_exp=dir_expr
                )
            )
            if fix_location:
                ...
                copy_command = "docker cp {} {}:{}".format(
                    tmp_file, container_id, target_path
                )
            ...
            save_command = "mkdir {}; cp {} {}".format(
                dir_expr + "/orig", dir_expr + "/src/" + fix_file, dir_expr + "/orig"
            )
            self.run_command(
                save_command, self.log_output_path, dir_expr + "/src", container_id
            )
            timestamp_command = (
    ...
    def save_artefacts(self, dir_info, experiment_info, container_id):
        ...
        copy_command = (
            "cp -rf  " + dir_preprocessed + " " + dir_artifact + "/preprocessed"
        )
        self.run_command(copy_command, "/dev/null", dir_expr, container_id)
