...
def read_arg(argument_list):
    ...
    if len(argument_list) > 0:
        for arg in argument_list:
            if definitions.ARG_DATA_PATH in arg:
                ...
                values.CONF_TOOL_NAME = (
                    str(arg).replace(definitions.ARG_TOOL_NAME, "").lower()
                )
            elif definitions.ARG_TOOL_LIST in arg:
                values.CONF_TOOL_LIST = (
                    str(arg).replace(definitions.ARG_TOOL_LIST, "").lower().split(",")
                )
            elif definitions.ARG_SUBJECT_NAME in arg:
                values.CONF_SUBJECT_NAME = (
                    str(arg).replace(definitions.ARG_SUBJECT_NAME, "").lower()
                )
                ...
                values.CONF_TOOL_PARAMS = str(arg).replace(
                ...
                values.CONF_CONFIG_ID_LIST = (
                    str(arg).replace(definitions.ARG_CONFIG_ID_LIST, "").split(",")
                )
            elif definitions.ARG_BUG_INDEX in arg:
                values.CONF_BUG_INDEX = int(
                    str(arg).replace(definitions.ARG_BUG_INDEX, "")
                )
                ...
                values.CONF_START_INDEX = int(
                    str(arg).replace(definitions.ARG_START_INDEX, "")
                )
            elif definitions.ARG_END_INDEX in arg:
                values.CONF_END_INDEX = int(
                    str(arg).replace(definitions.ARG_END_INDEX, "")
                )
                ...
                values.CONF_SKIP_LIST = (
                    str(arg).replace(definitions.ARG_SKIP_LIST, "").split(",")
                )
            elif definitions.ARG_BUG_INDEX_LIST in arg:
                values.CONF_BUG_INDEX_LIST = list(
                    flat_map(
                        convert_range,
                        str(arg).replace(definitions.ARG_BUG_INDEX_LIST, "").split(","),
                    )
                )
            elif definitions.ARG_BUG_ID_LIST in arg:
                values.CONF_BUG_ID_LIST = (
                    str(arg).replace(definitions.ARG_BUG_ID_LIST, "").split(",")
                )
    ...
    if values.CONF_SUBJECT_NAME:
        emitter.normal(
            "[info] running experiments for subject {}".format(values.CONF_SUBJECT_NAME)
        )
    if (
        values.CONF_START_INDEX is None
        and values.CONF_BUG_INDEX is None
        and values.CONF_BUG_INDEX_LIST is None
        and values.CONF_SUBJECT_NAME is None
    ):
        emitter.warning(
    ...
    if values.CONF_BENCHMARK is None:
        ...
        values.FILE_META_DATA = os.path.join(
            "benchmark", values.CONF_BENCHMARK, "meta-data.json"
        )
