...
if os.path.isfile(meta_file_path):
    with open(meta_file_path, "r") as in_file:
        ...
        for line in content:
            partition_data[partition_id].append(
    ...
    if os.path.isdir(patch_dir):
        output_patch_list = [
            f for f in listdir(patch_dir) if isfile(join(patch_dir, f))
        ]
        ...
        for patch_id in patch_data:
            ...
            if partition_id not in filtered_patch_list:
                ...
                sel_patch_id, min_syntax_dist, min_gen_order = filtered_patch_list[
                    partition_id
                ]
                if syntax_dist < min_syntax_dist:
                    filtered_patch_list[partition_id] = (
                        patch_id,
                        syntax_dist,
                        gen_order,
                    )
