...
with open(meta_file_path, "r") as meta_file:
    ...
    for bug_id in id_list:
        ...
        if not os.path.isdir(bug_id):
            init_command = "wget https://repairbenchmarks.cs.umass.edu/ManyBugs/scenarios/{id}.tar.gz; tar xf {id}.tar.gz".format(
                id=bug_id
            )
        ...
        if os.path.isdir(bug_id):
            ...
            with open(
                bug_fail_file_path, "r", encoding="utf8", errors="ignore"
            ) as log_file:
                bug_fail_list = log_file.readlines()
            with open(
                fix_fail_file_path, "r", encoding="utf8", errors="ignore"
            ) as log_file:
