...
#define DEFAULT_SUFFIX_LENGTH 2
 ...
 /* jump:189 */  if (split_type == type_chunk_bytes || split_type == type_chunk_lines || split_type == type_rr) {
	    uintmax_t n_units_end = n_units;
 /* jump:182 */    if (numeric_suffix_start) {
      uintmax_t n_start;
      strtol_error e = xstrtoumax(numeric_suffix_start, NULL, 10, &n_start, "");
 /* jump:181 */      if (e == LONGINT_OK && n_start <= UINTMAX_MAX - n_units) {
 ...
 /* jump:186 */    while (n_units_end /= alphabet_len)
      suffix_needed++;
 ...
 /* jump:195 */	    if (suffix_length < suffix_needed) {
      die(EXIT_FAILURE, 0,
          _("the suffix length needs to be at least %" PRIuMAX), suffix_needed);
    }
    suffix_auto = false;
    return;
  } else
...
"),
           program_name);
    fputs(_("\ Output pieces of FILE to PREFIXaa, PREFIXab, ...;\n\ default size is 1000 lines, and default PREFIX is 'x'.\n\ "),          stdout);
    ...
    fputs(_("\ --verbose           print a diagnostic just before each\n\ output file is opened\n\ "),          stdout);
...
static void next_file_name(void) {
 ...
 /* jump:307 */      outfile_length = outbase_length + suffix_length + addsuf_length;
    } else {
 ...
 /* jump:346 */    if (numeric_suffix_start) {
      assert(!widen);
 ...
 /* jump:372 */    while (i-- != 0) {
      sufindex[i]++;
      if (suffix_auto && i == 0 && !suffix_alphabet[sufindex[0] + 1])         goto new_name;
...
static int create(const char *name) {
 ...
 /* jump:429 */      if (fd_pair[0] != STDIN_FILENO) {
 ...
 /* jump:443 */    if (n_open_pipes == open_pipes_alloc)
      open_pipes =
...
static void closeout(FILE *fp, int fd, pid_t pid, char const *name) {
 ...
 /* jump:465 */  if (fd >= 0) {
    if (fp == NULL && close(fd) < 0)
      die(EXIT_FAILURE, errno, "%s", quotef(name));
    int j;
 /* jump:464 */    for (j = 0; j < n_open_pipes; ++j) {
 /* jump:463 */      if (open_pipes[j] == fd) {
 ...
 /* jump:479 */    if (WIFSIGNALED(wstatus)) {
      int sig = WTERMSIG(wstatus);
 /* jump:478 */      if (sig != SIGPIPE) {
 ...
 /* jump:484 */    } else if (WIFEXITED(wstatus)) {
...
static void bytes_split(uintmax_t n_bytes, char *buf, size_t bufsize,
 ...
 /* jump:567 */  do {
 /* jump:534 */    if (initial_read != SIZE_MAX) {
      n_read = initial_read;
 ...
 /* jump:573 */  while (opened++ < max_files)
    cwrite(true, NULL, 0);
}
...
static void lines_split(uintmax_t n_lines, char *buf, size_t bufsize) {
 ...
 /* jump:601 */      if (bp == eob) {
 /* jump:599 */        if (eob != bp_out) /* do not write 0 bytes! */ {
...
static void line_bytes_split(uintmax_t n_bytes, char *buf, size_t bufsize) {
 ...
 /* jump:644 */      if (n_bytes - n_out - n_hold <= n_left) {
 ...
 /* jump:694 */      if ((eoc && split_rest) || (!eoc && n_left)) {
        size_t n_buf = eoc ? split_rest : n_left;
 /* jump:689 */        if (hold_size - n_hold < n_buf) {
...
static void lines_chunk_split(uintmax_t k, uintmax_t n, char *buf,
 ...
 /* jump:737 */    if (start < initial_read) {
      memmove(buf, buf + start, initial_read - start);
      initial_read -= start;
    } else {
      if (lseek(STDIN_FILENO, start - initial_read, SEEK_CUR) < 0)
        die(EXIT_FAILURE, errno, "%s", quotef(infile));
 /* jump:741 */      initial_read = SIZE_MAX;
    }
    n_written = start;
    chunk_no = k - 1;
 ...
 /* jump:810 */  while (n_written < file_size) {
    char *bp = buf, *eob;
    size_t n_read;
 /* jump:753 */    if (initial_read != SIZE_MAX) {
      n_read = initial_read;
      initial_read = SIZE_MAX;
    } else {
      n_read = safe_read(STDIN_FILENO, buf, bufsize);
 ...
 /* jump:808 */      while (next || chunk_end <= n_written - 1) {
 /* jump:796 */        if (!next && bp == eob) {
...
static void bytes_chunk_extract(uintmax_t k, uintmax_t n, char *buf,
 ...
 /* jump:838 */  if (initial_read != SIZE_MAX || start < initial_read) {
    memmove(buf, buf + start, initial_read - start);
    initial_read -= start;
  } else {
 ...
 /* jump:859 */  while (start < end) {
    size_t n_read;
 /* jump:849 */    if (initial_read != SIZE_MAX) {
      n_read = initial_read;
      initial_read = SIZE_MAX;
    } else {
      n_read = safe_read(STDIN_FILENO, buf, bufsize);
...
static bool ofile_open(of_t *files, size_t i_check, size_t nfiles) {
 ...
 /* jump:935 */  if (files[i_check].ofd <= OFD_NEW) {
    int fd;
 ...
 /* jump:922 */      while (files[i_reopen].ofd < 0) {
...
static void lines_rr(uintmax_t k, uintmax_t n, char *buf, size_t bufsize) {
 ...
 /* jump:967 */    for (i_file = 0; i_file < n; i_file++) {
      next_file_name();
 ...
 /* jump:1037 */  while (true) {
    char *bp = buf, *eob;
    size_t n_read = safe_read(STDIN_FILENO, buf, bufsize);
    if (n_read == SAFE_READ_ERROR)
      die(EXIT_FAILURE, errno, "%s", quotef(infile));
    else if (n_read == 0)       break; /* eof.  */
    eob = buf + n_read;

 /* jump:1036 */    while (bp != eob) {
      size_t to_write;
      bool next = false;
 ...
 /* jump:1003 */      if (k) {
 /* jump:997 */        if (line_no == k && unbuffered) {
          if (full_write(STDOUT_FILENO, bp, to_write) != to_write)
            die(EXIT_FAILURE, errno, "%s", _("write error"));
 /* jump:1000 */        } else if (line_no == k && fwrite(bp, to_write, 1, stdout) != 1) {
 ...
 /* jump:1012 */        if (unbuffered) {
          /* Note writing to fd, rather than flushing the FILE gives
             an 8% performance benefit, due to reduced data copying.  */
 /* jump:1011 */          if (full_write(files[i_file].ofd, bp, to_write) != to_write && !ignorable(errno)) {
	            die(EXIT_FAILURE, errno, "%s", quotef(files[i_file].of_name));
          }
 /* jump:1014 */        } else if (fwrite(bp, to_write, 1, files[i_file].ofile) != 1 && !ignorable(errno)) {
	          die(EXIT_FAILURE, errno, "%s", quotef(files[i_file].of_name));
        }
 ...
 /* jump:1025 */        if (file_limit) {
 /* jump:1022 */          if (fclose(files[i_file].ofile) != 0) {
            die(EXIT_FAILURE, errno, "%s", quotef(files[i_file].of_name));
          }
          files[i_file].ofile = NULL;
          files[i_file].ofd = OFD_APPEND;
        }
 /* jump:1032 */        if (next && ++i_file == n) {
          wrapped = true;
...
no_filters:
 ...
 /* jump:1053 */    for (i_file = 0; i_file < n; i_file++) {
      if (i_file >= ceiling && !elide_empty_files)
        file_limit |= ofile_open(files, i_file, n);
 /* jump:1051 */      if (files[i_file].ofd >= 0)
...
static void parse_chunk(uintmax_t *k_units, uintmax_t *n_units, char *slash) {
 ...
 /* jump:1073 */  if (slash != optarg) /* a leading number is specified.  */ {
	    *slash = '\0';
    *k_units =
...
int main(int argc, char **argv) {
 ...
 /* jump:1120 */      if (last_component(optarg) != optarg) {
    ...
    case 'n':
      if (split_type != type_undef)
        FAIL_ONLY_ONE_WAY();
      /* skip any whitespace */
 /* jump:1156 */      while (isspace(to_uchar(*optarg)))
        optarg++;
 /* jump:1160 */      if (STRNCMP_LIT(optarg, "r/") == 0) {
        split_type = type_rr;
        optarg += 2;
 /* jump:1163 */      } else if (STRNCMP_LIT(optarg, "l/") == 0) {
 ...
 /* jump:1195 */      if (0 <= eolchar && neweol != eolchar) {
    ...
    case '8':
    case '9':
 /* jump:1213 */      if (split_type == type_undef) {
 ...
 /* jump:1223 */      if (!DECIMAL_DIGIT_ACCUMULATE(n_units, c - '0', uintmax_t)) {
 ...
 /* jump:1239 */      if (optarg) {
 /* jump:1233 */        if (strlen(optarg) != strspn(optarg, suffix_alphabet)) {
          error(0, 0, _("%s: invalid start value for numerical suffix"),
                quote(optarg));
          usage(EXIT_FAILURE);
        } else {
          /* Skip any leading zero.  */
 /* jump:1236 */          while (*optarg == '0' && *(optarg + 1) != '\0')
            optarg++;
 ...
 /* jump:1271 */  if (k_units != 0 && filter_command) {
    error(0, 0, _("--filter does not process a chunk extracted to stdout"));
    usage(EXIT_FAILURE);
  }

  /* Handle default case.  */
 /* jump:1277 */  if (split_type == type_undef) {
    split_type = type_lines;
    n_units = 1000;
  }

 /* jump:1282 */  if (n_units == 0) {
 ...
 /* jump:1300 */  if (optind < argc) {
  ...
  /* Check that the suffix length is large enough for the numerical
     suffix start value.  */
 /* jump:1307 */  if (numeric_suffix_start && strlen(numeric_suffix_start) > suffix_length) {
    error(0, 0, _("numerical suffix start value is too large "            "for the suffix length"));
    usage(EXIT_FAILURE);
  }
 ...
 /* jump:1345 */  if (split_type == type_chunk_bytes || split_type == type_chunk_lines) {
 ...
 /* jump:1341 */    if (OFF_T_MAX < n_units) {
      char buffer[INT_BUFSIZE_BOUND(uintmax_t)];
