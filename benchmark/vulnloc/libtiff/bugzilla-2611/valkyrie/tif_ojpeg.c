...
/* What is what, and what is not?
   ...
   other configuration mechanism to disable this behaviour, the actual header
 ...
 * anyway, and it should be at most 64K since it is indexed with uint16. We
...
typedef struct {
  ...
  uint32 bytes_per_line;   /* if the codec outputs subsampled data, a 'line' in
                              bytes_per_line */
  uint32 lines_per_strile; /* and lines_per_strile means subsampling_ver
                              desubsampled rows     */
  OJPEGStateInBufferSource in_buffer_source;
  uint32 in_buffer_next_strile;
...
static void OJPEGWriteStreamQTable(TIFF *tif, uint8 table_index, void **mem,
                                   uint32 *len);
static void OJPEGWriteStreamDcTable(TIFF *tif, uint8 table_index, void **mem,
                                    uint32 *len);
static void OJPEGWriteStreamAcTable(TIFF *tif, uint8 table_index, void **mem,
                                    uint32 *len);
...
int TIFFInitOJPEG(TIFF *tif, int scheme) {
   ...
   */
 /* jump:483 */  if (!_TIFFMergeFields(tif, ojpegFields, TIFFArrayCount(ojpegFields))) {
...
static int OJPEGVGetField(TIFF *tif, uint32 tag, va_list ap) {
 ...
 /* jump:542 */    if (sp->subsamplingcorrect_done == 0) {
...
static int OJPEGVSetField(TIFF *tif, uint32 tag, va_list ap) {
 ...
 /* jump:649 */  if (fip == NULL) { /* shouldn't happen */
    return (0);
  }
...
static void OJPEGPrintDir(TIFF *tif, FILE *fd, long flags) {
  ...
  (void)flags;
  assert(sp != NULL);
 /* jump:663 */  if (TIFFFieldSet(tif, FIELD_OJPEG_JPEGINTERCHANGEFORMAT)) {
    fprintf(fd, "  JpegInterchangeFormat: " TIFF_UINT64_FORMAT "\n",
            (TIFF_UINT64_T)sp->jpeg_interchange_format);
  }
 /* jump:667 */  if (TIFFFieldSet(tif, FIELD_OJPEG_JPEGINTERCHANGEFORMATLENGTH)) {
    fprintf(fd, "  JpegInterchangeFormatLength: " TIFF_UINT64_FORMAT "\n",
            (TIFF_UINT64_T)sp->jpeg_interchange_format_length);
  }
 /* jump:674 */  if (TIFFFieldSet(tif, FIELD_OJPEG_JPEGQTABLES)) {
    fprintf(fd, "  JpegQTables:");
 /* jump:672 */    for (m = 0; m < sp->qtable_offset_count; m++) {
      fprintf(fd, " " TIFF_UINT64_FORMAT, (TIFF_UINT64_T)sp->qtable_offset[m]);
    }
    fprintf(fd, "\n");
  }
 /* jump:681 */  if (TIFFFieldSet(tif, FIELD_OJPEG_JPEGDCTABLES)) {
    fprintf(fd, "  JpegDcTables:");
 /* jump:679 */    for (m = 0; m < sp->dctable_offset_count; m++) {
      fprintf(fd, " " TIFF_UINT64_FORMAT, (TIFF_UINT64_T)sp->dctable_offset[m]);
    }
    fprintf(fd, "\n");
  }
 /* jump:688 */  if (TIFFFieldSet(tif, FIELD_OJPEG_JPEGACTABLES)) {
    fprintf(fd, "  JpegAcTables:");
 /* jump:686 */    for (m = 0; m < sp->actable_offset_count; m++) {
      fprintf(fd, " " TIFF_UINT64_FORMAT, (TIFF_UINT64_T)sp->actable_offset[m]);
    }
    fprintf(fd, "\n");
  }
 /* jump:691 */  if (TIFFFieldSet(tif, FIELD_OJPEG_JPEGPROC)) {
    fprintf(fd, "  JpegProc: %u\n", (unsigned int)sp->jpeg_proc);
  }
 /* jump:695 */  if (TIFFFieldSet(tif, FIELD_OJPEG_JPEGRESTARTINTERVAL)) {
    fprintf(fd, "  JpegRestartInterval: %u\n",
            (unsigned int)sp->restart_interval);
  }
 /* jump:698 */  if (sp->printdir) {
...
static int OJPEGSetupDecode(TIFF *tif) {
  ...
  TIFFWarningExt(tif->tif_clientdata, module,
                 "Depreciated and troublesome old-style JPEG compression mode, "
                 "please convert to new-style JPEG compression and notify "
...
static int OJPEGPreDecode(TIFF *tif, uint16 s) {
  OJPEGState *sp = (OJPEGState *)tif->tif_data;
  uint32 m;
 /* jump:720 */  if (sp->subsamplingcorrect_done == 0) {
    OJPEGSubsamplingCorrect(tif);
  }
 /* jump:725 */  if (sp->readheader_done == 0) {
 /* jump:724 */    if (OJPEGReadHeaderInfo(tif) == 0) {
      return (0);
    }
  }
 /* jump:730 */  if (sp->sos_end[s].log == 0) {
 /* jump:729 */    if (OJPEGReadSecondarySos(tif, s) == 0) {
      return (0);
    }
  }
  if isTiled (tif)
    ...
    m = tif->tif_curstrip;
 /* jump:741 */  if ((sp->writeheader_done != 0) &&
      ((sp->write_cursample != s) || (sp->write_curstrile > m))) {
 /* jump:739 */    if (sp->libjpeg_session_active != 0) {
      OJPEGLibjpegSessionAbort(tif);
    }
    sp->writeheader_done = 0;
  }
 /* jump:760 */  if (sp->writeheader_done == 0) {
    sp->plane_sample_offset = (uint8)s;
    sp->write_cursample = s;
    sp->write_curstrile = s * tif->tif_dir.td_stripsperimage;
 /* jump:756 */    if ((sp->in_buffer_file_pos_log == 0) ||
 ...
 /* jump:759 */    if (OJPEGWriteHeaderInfo(tif) == 0) {
      return (0);
    }
  }
 ...
 /* jump:769 */      if (OJPEGPreDecodeSkipScanlines(tif) == 0) {
        return (0);
      }
    }
    sp->write_curstrile++;
...
static int OJPEGPreDecodeSkipRaw(TIFF *tif) {
 ...
 /* jump:790 */  if (sp->subsampling_convert_state != 0) {
 /* jump:787 */    if (sp->subsampling_convert_clines - sp->subsampling_convert_state >= m) {
      sp->subsampling_convert_state += m;
 /* jump:785 */      if (sp->subsampling_convert_state == sp->subsampling_convert_clines) {
 ...
 /* jump:798 */  while (m >= sp->subsampling_convert_clines) {
 /* jump:796 */    if (jpeg_read_raw_data_encap(sp, &(sp->libjpeg_jpeg_decompress_struct),
 ...
 /* jump:806 */  if (m > 0) {
 /* jump:804 */    if (jpeg_read_raw_data_encap(sp, &(sp->libjpeg_jpeg_decompress_struct),
                                 sp->subsampling_convert_ycbcrimage,
                                 sp->subsampling_ver * 8) == 0) {
      return (0);
    }
...
static int OJPEGPreDecodeSkipScanlines(TIFF *tif) {
  ...
  OJPEGState *sp = (OJPEGState *)tif->tif_data;
  uint32 m;
 /* jump:820 */  if (sp->skip_buffer == NULL) {
    sp->skip_buffer = _TIFFmalloc(sp->bytes_per_line);
 /* jump:819 */    if (sp->skip_buffer == NULL) {
      TIFFErrorExt(tif->tif_clientdata, module, "Out of memory");
      return (0);
    }
  }
 /* jump:826 */  for (m = 0; m < sp->lines_per_strile; m++) {
 /* jump:825 */    if (jpeg_read_scanlines_encap(sp, &(sp->libjpeg_jpeg_decompress_struct),
                                  &sp->skip_buffer, 1) == 0) {
      return (0);
    }
  }
...
static int OJPEGDecode(TIFF *tif, uint8 *buf, tmsize_t cc, uint16 s) {
 ...
 /* jump:840 */    if (OJPEGDecodeScanlines(tif, buf, cc) == 0) {
      return (0);
    }
  }
  return (1);
...
static int OJPEGDecodeRaw(TIFF *tif, uint8 *buf, tmsize_t cc) {
  ...
  uint8 *r;
  uint8 sx, sy;
 /* jump:860 */  if (cc % sp->bytes_per_line != 0) {
 ...
 /* jump:870 */      if (jpeg_read_raw_data_encap(sp, &(sp->libjpeg_jpeg_decompress_struct),
    ...
    oy = sp->subsampling_convert_ybuf + sp->subsampling_convert_state *
                                            sp->subsampling_ver *
    ...
    ocr = sp->subsampling_convert_crbuf +
          sp->subsampling_convert_state * sp->subsampling_convert_clinelen;
    p = m;
 /* jump:891 */    for (q = 0; q < sp->subsampling_convert_clinelenout; q++) {
      r = oy;
 /* jump:887 */      for (sy = 0; sy < sp->subsampling_ver; sy++) {
 /* jump:885 */        for (sx = 0; sx < sp->subsampling_hor; sx++) {
          *p++ = *r++;
        }
 ...
 /* jump:895 */    if (sp->subsampling_convert_state == sp->subsampling_convert_clines) {
      sp->subsampling_convert_state = 0;
    }
    m += sp->bytes_per_line;
    n -= sp->bytes_per_line;
...
static int OJPEGDecodeScanlines(TIFF *tif, uint8 *buf, tmsize_t cc) {
  ...
  uint8 *m;
  tmsize_t n;
 /* jump:910 */  if (cc % sp->bytes_per_line != 0) {
    TIFFErrorExt(tif->tif_clientdata, module, "Fractional scanline not read");
    return (0);
  }
  assert(cc > 0);
...
static void OJPEGPostDecode(TIFF *tif, uint8 *buf, tmsize_t cc) {
  ...
  (void)cc;
  sp->write_curstrile++;
 /* jump:934 */  if (sp->write_curstrile % tif->tif_dir.td_stripsperimage == 0) {
...
static void OJPEGCleanup(TIFF *tif) {
 ...
 /* jump:981 */    if (sp->qtable[0] != 0) {
      _TIFFfree(sp->qtable[0]);
    }
 /* jump:984 */    if (sp->qtable[1] != 0) {
      _TIFFfree(sp->qtable[1]);
    }
 /* jump:987 */    if (sp->qtable[2] != 0) {
      _TIFFfree(sp->qtable[2]);
    }
 /* jump:990 */    if (sp->qtable[3] != 0) {
      _TIFFfree(sp->qtable[3]);
    }
 /* jump:993 */    if (sp->dctable[0] != 0) {
      _TIFFfree(sp->dctable[0]);
    }
 /* jump:996 */    if (sp->dctable[1] != 0) {
      _TIFFfree(sp->dctable[1]);
    }
 /* jump:999 */    if (sp->dctable[2] != 0) {
      _TIFFfree(sp->dctable[2]);
    }
 /* jump:1002 */    if (sp->dctable[3] != 0) {
      _TIFFfree(sp->dctable[3]);
    }
 /* jump:1005 */    if (sp->actable[0] != 0) {
      _TIFFfree(sp->actable[0]);
    }
 /* jump:1008 */    if (sp->actable[1] != 0) {
      _TIFFfree(sp->actable[1]);
    }
 /* jump:1011 */    if (sp->actable[2] != 0) {
      _TIFFfree(sp->actable[2]);
    }
 /* jump:1014 */    if (sp->actable[3] != 0) {
      _TIFFfree(sp->actable[3]);
    }
 /* jump:1017 */    if (sp->libjpeg_session_active != 0) {
      OJPEGLibjpegSessionAbort(tif);
    }
 /* jump:1020 */    if (sp->subsampling_convert_ycbcrbuf != 0) {
      _TIFFfree(sp->subsampling_convert_ycbcrbuf);
    }
 /* jump:1023 */    if (sp->subsampling_convert_ycbcrimage != 0) {
      _TIFFfree(sp->subsampling_convert_ycbcrimage);
    }
 /* jump:1026 */    if (sp->skip_buffer != 0) {
...
static void OJPEGSubsamplingCorrect(TIFF *tif) {
 ...
 /* jump:1048 */    if (sp->subsampling_tag != 0) {
      TIFFWarningExt(tif->tif_clientdata, module,
                     "Subsampling tag not appropriate for this Photometric "
                     "and/or SamplesPerPixel");
    }
 ...
 /* jump:1061 */    if (sp->subsampling_force_desubsampling_inside_decompression != 0) {
      sp->subsampling_hor = 1;
      sp->subsampling_ver = 1;
    }
 ...
 /* jump:1071 */      if (sp->subsampling_tag == 0) {
        ...
        TIFFWarningExt(tif->tif_clientdata, module,
                       "Subsampling inside JPEG data [%d,%d] does not match "
                       "subsampling tag values [%d,%d]; assuming subsampling "
                       "inside JPEG data is correct",
 ...
 /* jump:1087 */      if (sp->subsampling_tag == 0) {
        TIFFWarningExt(
            ...
            "not match default values [2,2] (nor any other values allowed in "
            "TIFF); assuming subsampling inside JPEG data is correct and "
            "desubsampling inside JPEG decompression");
      } else {
        TIFFWarningExt(
            tif->tif_clientdata, module,
            "Subsampling inside JPEG data does not match subsampling tag "
            "values [%d,%d] (nor any other values allowed in TIFF); assuming "
            "subsampling inside JPEG data is correct and desubsampling inside "
            "JPEG decompression",
            mh, mv);
      }
    }
 /* jump:1103 */    if (sp->subsampling_force_desubsampling_inside_decompression == 0) {
 /* jump:1102 */      if (sp->subsampling_hor < sp->subsampling_ver) {
        TIFFWarningExt(tif->tif_clientdata, module,
...
static int OJPEGReadHeaderInfo(TIFF *tif) {
 ...
 /* jump:1138 */    if (tif->tif_dir.td_samplesperpixel != 3) {
      TIFFErrorExt(
          tif->tif_clientdata, module,
 ...
 /* jump:1158 */  if (sp->strile_length < sp->image_length) {
 /* jump:1153 */    if (sp->strile_length % (sp->subsampling_ver * 8) != 0) {
      TIFFErrorExt(
          tif->tif_clientdata, module,
    ...
    sp->restart_interval =
        (uint16)(((sp->strile_width + sp->subsampling_hor * 8 - 1) /
                  (sp->subsampling_hor * 8)) *
                 (sp->strile_length / (sp->subsampling_ver * 8)));
  }
 /* jump:1161 */  if (OJPEGReadHeaderInfoSec(tif) == 0) {
    return (0);
  }
  sp->sos_end[0].log = 1;
...
static int OJPEGReadSecondarySos(TIFF *tif, uint16 s) {
 ...
 /* jump:1183 */  while (sp->sos_end[sp->plane_sample_offset].log == 0) {
 ...
 /* jump:1210 */        if (m == JPEG_MARKER_SOS) {
          break;
        }
      }
    } while (1);
    sp->plane_sample_offset++;
 /* jump:1216 */    if (OJPEGReadHeaderInfoSecStreamSos(tif) == 0) {
      return (0);
    }
...
static int OJPEGWriteHeaderInfo(TIFF *tif) {
 ...
 /* jump:1238 */  if (sp->libjpeg_session_active != 0) {
    return 0;
  }
 ...
 /* jump:1250 */  if (jpeg_create_decompress_encap(sp, &(sp->libjpeg_jpeg_decompress_struct)) ==
      0) {
    return (0);
  }
 ...
 /* jump:1265 */  if (jpeg_read_header_encap(sp, &(sp->libjpeg_jpeg_decompress_struct), 1) ==
      0) {
    return (0);
  }
 ...
 /* jump:1336 */    if (sp->subsampling_convert_log == 0) {
      ...
      sp->subsampling_convert_ycbcrbuf =
          _TIFFmalloc(sp->subsampling_convert_ycbcrbuflen);
 /* jump:1294 */      if (sp->subsampling_convert_ycbcrbuf == 0) {
        TIFFErrorExt(tif->tif_clientdata, module, "Out of memory");
        return (0);
      }
      ...
      sp->subsampling_convert_ycbcrimage =
          _TIFFmalloc(sp->subsampling_convert_ycbcrimagelen * sizeof(uint8 *));
 /* jump:1308 */      if (sp->subsampling_convert_ycbcrimage == 0) {
        TIFFErrorExt(tif->tif_clientdata, module, "Out of memory");
        return (0);
      }
 ...
 /* jump:1319 */      for (n = 0; n < sp->subsampling_convert_ylines; n++) {
        *m++ =
            sp->subsampling_convert_ybuf + n * sp->subsampling_convert_ylinelen;
      }
 /* jump:1323 */      for (n = 0; n < sp->subsampling_convert_clines; n++) {
        *m++ = sp->subsampling_convert_cbbuf +
               n * sp->subsampling_convert_clinelen;
      }
 /* jump:1327 */      for (n = 0; n < sp->subsampling_convert_clines; n++) {
        *m++ = sp->subsampling_convert_crbuf +
               n * sp->subsampling_convert_clinelen;
      }
      sp->subsampling_convert_clinelenout =
          ((sp->strile_width + sp->subsampling_hor - 1) / sp->subsampling_hor);
      sp->subsampling_convert_state = 0;
      sp->bytes_per_line = sp->subsampling_convert_clinelenout *
                           (sp->subsampling_ver * sp->subsampling_hor + 2);
      sp->lines_per_strile =
          ((sp->strile_length + sp->subsampling_ver - 1) / sp->subsampling_ver);
 ...
 /* jump:1347 */  if (jpeg_start_decompress_encap(sp, &(sp->libjpeg_jpeg_decompress_struct)) ==
      0) {
    return (0);
  }
...
static int OJPEGReadHeaderInfoSec(TIFF *tif) {
 ...
 /* jump:1378 */      if ((sp->jpeg_interchange_format_length == 0) ||
          (sp->jpeg_interchange_format + sp->jpeg_interchange_format_length >
           sp->file_size)) {
 ...
 /* jump:1397 */      if (OJPEGReadByte(sp, &m) == 0) {
    ...
    case JPEG_MARKER_APP0 + 15:
      /* this type of marker has data, but it has no use to us (and no place
       * here) and should be skipped */
 /* jump:1424 */      if (OJPEGReadWord(sp, &n) == 0) {
        return (0);
      }
 /* jump:1430 */      if (n < 2) {
 /* jump:1428 */        if (sp->subsamplingcorrect == 0) {
 ...
 /* jump:1438 */      if (OJPEGReadHeaderInfoSecStreamDri(tif) == 0) {
        return (0);
      }
      break;
    case JPEG_MARKER_DQT:
 /* jump:1443 */      if (OJPEGReadHeaderInfoSecStreamDqt(tif) == 0) {
        return (0);
      }
      break;
    case JPEG_MARKER_DHT:
 /* jump:1448 */      if (OJPEGReadHeaderInfoSecStreamDht(tif) == 0) {
        return (0);
      }
      break;
 ...
 /* jump:1455 */      if (OJPEGReadHeaderInfoSecStreamSof(tif, m) == 0) {
        return (0);
      }
 /* jump:1458 */      if (sp->subsamplingcorrect != 0) {
        return (1);
      }
      break;
    case JPEG_MARKER_SOS:
 /* jump:1463 */      if (sp->subsamplingcorrect != 0) {
        return (1);
      }
      assert(sp->plane_sample_offset == 0);
 /* jump:1467 */      if (OJPEGReadHeaderInfoSecStreamSos(tif) == 0) {
        return (0);
      }
      break;
    default:
 ...
 /* jump:1477 */  if (sp->subsamplingcorrect) {
    return (1);
  }
 /* jump:1502 */  if (sp->sof_log == 0) {
 /* jump:1481 */    if (OJPEGReadHeaderInfoSecTablesQTable(tif) == 0) {
      return (0);
    }
    sp->sof_marker_id = JPEG_MARKER_SOF0;
 /* jump:1485 */    for (o = 0; o < sp->samples_per_pixel; o++) {
      sp->sof_c[o] = o;
    }
    sp->sof_hv[0] = ((sp->subsampling_hor << 4) | sp->subsampling_ver);
 /* jump:1489 */    for (o = 1; o < sp->samples_per_pixel; o++) {
      sp->sof_hv[o] = 17;
    }
    sp->sof_x = sp->strile_width;
    sp->sof_y = sp->strile_length_total;
    sp->sof_log = 1;
 /* jump:1495 */    if (OJPEGReadHeaderInfoSecTablesDcTable(tif) == 0) {
      return (0);
    }
 /* jump:1498 */    if (OJPEGReadHeaderInfoSecTablesAcTable(tif) == 0) {
      return (0);
    }
 /* jump:1501 */    for (o = 1; o < sp->samples_per_pixel; o++) {
      sp->sos_cs[o] = o;
    }
  }
  return (1);
...
static int OJPEGReadHeaderInfoSecStreamDri(TIFF *tif) {
  ...
  OJPEGState *sp = (OJPEGState *)tif->tif_data;
  uint16 m;
 /* jump:1514 */  if (OJPEGReadWord(sp, &m) == 0) {
    return (0);
  }
 /* jump:1519 */  if (m != 4) {
    TIFFErrorExt(tif->tif_clientdata, module,
 ...
 /* jump:1522 */  if (OJPEGReadWord(sp, &m) == 0) {
    return (0);
  }
...
static int OJPEGReadHeaderInfoSecStreamDqt(TIFF *tif) {
  /* this is a table marker, and it is to be saved as a whole for exact pushing
   * on the jpeg stream later on */
  ...
  uint8 *nb;
  uint8 o;
 /* jump:1538 */  if (OJPEGReadWord(sp, &m) == 0) {
    return (0);
  }
 /* jump:1545 */  if (m <= 2) {
 /* jump:1543 */    if (sp->subsamplingcorrect == 0) {
      TIFFErrorExt(tif->tif_clientdata, module,
 ...
 /* jump:1570 */      if (OJPEGReadBlock(sp, 65, &nb[sizeof(uint32) + 4]) == 0) {
        _TIFFfree(nb);
        return (0);
      }
...
static int OJPEGReadHeaderInfoSecStreamDht(TIFF *tif) {
  ...
  uint8 *nb;
  uint8 o;
 /* jump:1601 */  if (OJPEGReadWord(sp, &m) == 0) {
    return (0);
  }
 /* jump:1608 */  if (m <= 2) {
 /* jump:1606 */    if (sp->subsamplingcorrect == 0) {
      TIFFErrorExt(tif->tif_clientdata, module,
 ...
 /* jump:1626 */    if (OJPEGReadBlock(sp, m - 2, &nb[sizeof(uint32) + 4]) == 0) {
      _TIFFfree(nb);
      return (0);
    }
 ...
 /* jump:1637 */      if (sp->dctable[o] != 0) {
        _TIFFfree(sp->dctable[o]);
      }
      sp->dctable[o] = nb;
    } else {
 /* jump:1645 */      if ((o & 240) != 16) {
        TIFFErrorExt(tif->tif_clientdata, module,
                     "Corrupt DHT marker in JPEG data");
        _TIFFfree(nb);
        return (0);
...
static int OJPEGReadHeaderInfoSecStreamSof(TIFF *tif, uint8 marker_id) {
  /* this marker needs to be checked, and part of its data needs to be saved for
  ...
  uint16 p;
  uint16 q;
 /* jump:1675 */  if (sp->sof_log != 0) {
    TIFFErrorExt(tif->tif_clientdata, module, "Corrupt JPEG data");
    return (0);
  }
 /* jump:1678 */  if (sp->subsamplingcorrect == 0) {
    sp->sof_marker_id = marker_id;
  }
  /* Lf: data length */
 /* jump:1682 */  if (OJPEGReadWord(sp, &m) == 0) {
    return (0);
  }
 /* jump:1689 */  if (m < 11) {
 /* jump:1687 */    if (sp->subsamplingcorrect == 0) {
      TIFFErrorExt(tif->tif_clientdata, module,
  ...
  m -= 8;
 /* jump:1697 */  if (m % 3 != 0) {
 /* jump:1695 */    if (sp->subsamplingcorrect == 0) {
      TIFFErrorExt(tif->tif_clientdata, module,
                   "Corrupt SOF marker in JPEG data");
    }
    return (0);
  ...
  n = m / 3;
 /* jump:1706 */  if (sp->subsamplingcorrect == 0) {
 /* jump:1705 */    if (n != sp->samples_per_pixel) {
      TIFFErrorExt(
          tif->tif_clientdata, module,
  ...
  /* P: Sample precision */
 /* jump:1710 */  if (OJPEGReadByte(sp, &o) == 0) {
    return (0);
  }
 /* jump:1718 */  if (o != 8) {
 /* jump:1716 */    if (sp->subsamplingcorrect == 0) {
      TIFFErrorExt(tif->tif_clientdata, module,
                   "JPEG compressed data indicates unexpected number of bits "
                   "per sample");
    }
    return (0);
  }
 ...
 /* jump:1722 */  if (sp->subsamplingcorrect) {
    ...
    /* Y: Number of lines */
 /* jump:1726 */    if (OJPEGReadWord(sp, &p) == 0) {
      return (0);
    }
 /* jump:1732 */    if (((uint32)p < sp->image_length) &&
        ((uint32)p < sp->strile_length_total)) {
      TIFFErrorExt(tif->tif_clientdata, module,
 ...
 /* jump:1737 */    if (OJPEGReadWord(sp, &p) == 0) {
      return (0);
    }
 /* jump:1742 */    if (((uint32)p < sp->image_width) && ((uint32)p < sp->strile_width)) {
      TIFFErrorExt(tif->tif_clientdata, module,
                   "JPEG compressed data indicates unexpected width");
      return (0);
    }
 /* jump:1748 */    if ((uint32)p > sp->strile_width) {
      TIFFErrorExt(
          tif->tif_clientdata, module,
 ...
 /* jump:1754 */  if (OJPEGReadByte(sp, &o) == 0) {
    return (0);
  }
 /* jump:1761 */  if (o != n) {
 /* jump:1759 */    if (sp->subsamplingcorrect == 0) {
      TIFFErrorExt(tif->tif_clientdata, module,
                   "Corrupt SOF marker in JPEG data");
    }
    return (0);
  ...
  /* per component stuff */
  /* TODO: double-check that flow implies that n cannot be as big as to make us
   * overflow sof_c, sof_hv and sof_tq arrays */
 /* jump:1819 */  for (q = 0; q < n; q++) {
    /* C: Component identifier */
 /* jump:1769 */    if (OJPEGReadByte(sp, &o) == 0) {
      return (0);
    }
 /* jump:1772 */    if (sp->subsamplingcorrect == 0) {
      sp->sof_c[q] = o;
    }
 ...
 /* jump:1815 */    if (OJPEGReadByte(sp, &o) == 0) {
      return (0);
    }
 /* jump:1818 */    if (sp->subsamplingcorrect == 0) {
      sp->sof_tq[q] = o;
    }
  }
 /* jump:1822 */  if (sp->subsamplingcorrect == 0) {
    sp->sof_log = 1;
  }
  return (1);
}
...
static int OJPEGReadHeaderInfoSecStreamSos(TIFF *tif) {
 ...
 /* jump:1839 */  if (sp->sof_log == 0) {
    TIFFErrorExt(tif->tif_clientdata, module,
                 "Corrupt SOS marker in JPEG data");
    return (0);
  }
  /* Ls */
 /* jump:1843 */  if (OJPEGReadWord(sp, &m) == 0) {
    return (0);
  }
 /* jump:1848 */  if (m != 6 + sp->samples_per_pixel_per_plane * 2) {
    TIFFErrorExt(tif->tif_clientdata, module,
                 "Corrupt SOS marker in JPEG data");
    return (0);
  }
  /* Ns */
 /* jump:1852 */  if (OJPEGReadByte(sp, &n) == 0) {
    return (0);
  }
 /* jump:1857 */  if (n != sp->samples_per_pixel_per_plane) {
    TIFFErrorExt(tif->tif_clientdata, module,
                 "Corrupt SOS marker in JPEG data");
    return (0);
  }
  /* Cs, Td, and Ta */
 /* jump:1870 */  for (o = 0; o < sp->samples_per_pixel_per_plane; o++) {
    /* Cs */
 /* jump:1863 */    if (OJPEGReadByte(sp, &n) == 0) {
      return (0);
    }
    sp->sos_cs[sp->plane_sample_offset + o] = n;
    /* Td and Ta */
 /* jump:1868 */    if (OJPEGReadByte(sp, &n) == 0) {
      return (0);
    }
    sp->sos_tda[sp->plane_sample_offset + o] = n;
  }
  /* skip Ss, Se, Ah, en Al -> no check, as per Tom Lane recommendation, as per
   * LibJpeg source */
  OJPEGReadSkip(sp, 3);
  return (1);
}
...
static int OJPEGReadHeaderInfoSecTablesQTable(TIFF *tif) {
  ...
  uint8 *ob;
  uint32 p;
 /* jump:1888 */  if (sp->qtable_offset[0] == 0) {
...
static int OJPEGReadHeaderInfoSecTablesDcTable(TIFF *tif) {
  ...
  uint32 ra;
  uint8 *rb;
 /* jump:1939 */  if (sp->dctable_offset[0] == 0) {
    TIFFErrorExt(tif->tif_clientdata, module, "Missing JPEG tables");
    return (0);
  }
  sp->in_buffer_file_pos_log = 0;
...
static int OJPEGReadHeaderInfoSecTablesAcTable(TIFF *tif) {
  ...
  uint32 ra;
  uint8 *rb;
 /* jump:2001 */  if (sp->actable_offset[0] == 0) {
    TIFFErrorExt(tif->tif_clientdata, module, "Missing JPEG tables");
    return (0);
  }
  sp->in_buffer_file_pos_log = 0;
...
static int OJPEGReadBufferFill(OJPEGState *sp) {
 ...
 /* jump:2061 */      if (sp->in_buffer_file_pos_log == 0) {
      ...
      m = OJPEG_BUFFER;
 /* jump:2065 */      if ((uint64)m > sp->in_buffer_file_togo) {
 ...
 /* jump:2069 */      if (n == 0) {
    ...
    case osibsNotSetYet:
 /* jump:2087 */      if (sp->jpeg_interchange_format != 0) {
    ...
    case osibsJpegInterchangeFormat:
      ...
      break;
    case osibsStrile:
 /* jump:2098 */      if (!_TIFFFillStriles(sp->tif) ||
 ...
 /* jump:2115 */            if (sp->tif->tif_dir.td_stripbytecount == 0) {
              TIFFErrorExt(sp->tif->tif_clientdata, sp->tif->tif_name,
...
static int OJPEGReadByte(OJPEGState *sp, uint8 *byte) {
 /* jump:2142 */  if (sp->in_buffer_togo == 0) {
 /* jump:2140 */    if (OJPEGReadBufferFill(sp) == 0) {
      return (0);
    }
...
static int OJPEGReadBytePeek(OJPEGState *sp, uint8 *byte) {
 /* jump:2155 */  if (sp->in_buffer_togo == 0) {
 /* jump:2153 */    if (OJPEGReadBufferFill(sp) == 0) {
      return (0);
    }
    assert(sp->in_buffer_togo > 0);
  }
...
static int OJPEGReadWord(OJPEGState *sp, uint16 *word) {
  uint8 m;
 /* jump:2170 */  if (OJPEGReadByte(sp, &m) == 0) {
    return (0);
  }
  *word = (m << 8);
 /* jump:2174 */  if (OJPEGReadByte(sp, &m) == 0) {
    return (0);
  }
  *word |= m;
  return (1);
...
static int OJPEGReadBlock(OJPEGState *sp, uint16 len, void *mem) {
 ...
 /* jump:2190 */      if (OJPEGReadBufferFill(sp) == 0) {
    ...
    n = mlen;
 /* jump:2196 */    if (n > sp->in_buffer_togo) {
      n = sp->in_buffer_togo;
    }
...
static void OJPEGReadSkip(OJPEGState *sp, uint16 len) {
  ...
  m = len;
  n = m;
 /* jump:2213 */  if (n > sp->in_buffer_togo) {
    n = sp->in_buffer_togo;
  }
  sp->in_buffer_cur += n;
  sp->in_buffer_togo -= n;
 ...
 /* jump:2232 */  if (m > 0) {
    assert(sp->in_buffer_togo == 0);
    n = m;
 /* jump:2222 */    if ((uint64)n > sp->in_buffer_file_togo) {
...
static int OJPEGWriteStream(TIFF *tif, void **mem, uint32 *len) {
 ...
 /* jump:2301 */  do {
    ...
    case ososSos:
      ...
      break;
    case ososCompressed:
 /* jump:2292 */      if (OJPEGWriteStreamCompressed(tif, mem, len) == 0) {
        return (0);
      }
      break;
    case ososRst:
...
static void OJPEGWriteStreamQTable(TIFF *tif, uint8 table_index, void **mem,
                                   uint32 *len) {
  OJPEGState *sp = (OJPEGState *)tif->tif_data;
 /* jump:2321 */  if (sp->qtable[table_index] != 0) {
...
static void OJPEGWriteStreamDcTable(TIFF *tif, uint8 table_index, void **mem,
                                    uint32 *len) {
  OJPEGState *sp = (OJPEGState *)tif->tif_data;
 /* jump:2331 */  if (sp->dctable[table_index] != 0) {
...
static void OJPEGWriteStreamAcTable(TIFF *tif, uint8 table_index, void **mem,
                                    uint32 *len) {
  OJPEGState *sp = (OJPEGState *)tif->tif_data;
 /* jump:2341 */  if (sp->actable[table_index] != 0) {
...
static void OJPEGWriteStreamDri(TIFF *tif, void **mem, uint32 *len) {
 ...
 /* jump:2357 */  if (sp->restart_interval != 0) {
    sp->out_buffer[0] = 255;
...
static void OJPEGWriteStreamSof(TIFF *tif, void **mem, uint32 *len) {
 ...
 /* jump:2388 */  for (m = 0; m < sp->samples_per_pixel_per_plane; m++) {
    /* C */
...
static void OJPEGWriteStreamSos(TIFF *tif, void **mem, uint32 *len) {
 ...
 /* jump:2411 */  for (m = 0; m < sp->samples_per_pixel_per_plane; m++) {
    /* Cs */
...
static int OJPEGWriteStreamCompressed(TIFF *tif, void **mem, uint32 *len) {
  OJPEGState *sp = (OJPEGState *)tif->tif_data;
 /* jump:2430 */  if (sp->in_buffer_togo == 0) {
 /* jump:2428 */    if (OJPEGReadBufferFill(sp) == 0) {
      return (0);
    }
    assert(sp->in_buffer_togo > 0);
  }
...
static void OJPEGWriteStreamRst(TIFF *tif, void **mem, uint32 *len) {
  ...
  sp->restart_index++;
 /* jump:2461 */  if (sp->restart_index == 8) {
    sp->restart_index = 0;
  }
  *len = 2;
  *mem = (void *)sp->out_buffer;
...
static void OJPEGLibjpegJpegErrorMgrOutputMessage(jpeg_common_struct *cinfo) {
  ...
  TIFFWarningExt(((TIFF *)(cinfo->client_data))->tif_clientdata, "LibJpeg",
                 "%s", buffer);
}
...
static void OJPEGLibjpegJpegErrorMgrErrorExit(jpeg_common_struct *cinfo) {
  char buffer[JMSG_LENGTH_MAX];
  (*cinfo->err->format_message)(cinfo, buffer);
  TIFFErrorExt(((TIFF *)(cinfo->client_data))->tif_clientdata, "LibJpeg", "%s",
               buffer);
...
OJPEGLibjpegJpegSourceMgrFillInputBuffer(jpeg_decompress_struct *cinfo) {
  ...
  void *mem = 0;
  uint32 len = 0U;
 /* jump:2573 */  if (OJPEGWriteStream(tif, &mem, &len) == 0) {
