...
then
    ...
    cflags="$cflags -fsanitize=lowfat -mllvm -lowfat-debug -mllvm -lowfat-no-check-memset -mllvm -lowfat-no-check-memcpy -mllvm -lowfat-no-check-escapes -mllvm -lowfat-no-replace-globals -mllvm -lowfat-no-check-fields -mllvm -lowfat-symbolize -lstlimpl"
fi
