...
void comb_sort(int N,int arr[])
{
    ...
    while(1)
    {
        ...
        {
            if(arr[i]>arr[i+gap])//Condition for swapping
            {
                t=arr[i];
                arr[i]=arr[i+gap];
                arr[i+gap]=t;
        ...
        if(c==0&&gap==1)//Condition for exiting the loop(Array is sorted)
        break;
        gap=gap/shrink;
    }
    for(int i=0;i<N;i++)
...
int main()
{
    int n,*ar;
    scanf("%d\n",&n);
    ar=(int*)malloc(n*sizeof(int));//Dynamic Initialisation
    for(int i=0;i<n;i++)
        scanf("%d ",&ar[i]);
    comb_sort(n,ar); //Calling the function
    return 0;
}